...
// const branches = HQBranch.find({}, (err, list) => {
//     if (err) return err;
//     return list;
// });
...
router.post('/', async (req, res) => {
    fs.readFileSync('PNOV0213.ASC').toString().split(/\r?\n/).forEach(async function(line){
        const company = {
            ...
            sex: _.isEmpty(line.substring(881, 882).trim()) ? line.substring(881, 882) : null,
            rsi_sex: line.substring(882, 883).trim(),
            domain_name: line.substring(904, 950).trim(),
            rsi_domain_name: line.substring(951, 952).trim()
        };
        const created = await (new Company(company)).save();
        console.log(created);
    });
    res.json("All data read successfuly");
});
